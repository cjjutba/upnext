...
describe('roster commands', () => {
  ...
  it('sitOutPlayer freezes, returnPlayer unfreezes and can trigger a fill', () => {
    let log = boot(['a', 'b', 'c', 'd'], 'all-off', 1);
    log = [...log, ...seal(finishGame(replay(log), 1)!)];
    let s = replay(log);
    log = [...log, ...seal(sitOutPlayer(s, s.queue[0])!)];
    s = replay(log);
    expect(s.sittingOut).toHaveLength(1);
    expect(s.games[1]).toBeUndefined(); // only three eligible
    log = [...log, ...seal(returnPlayer(s, s.sittingOut[0])!)];
    expect(replay(log).games[1]).toBeDefined();
  });
...
/** game-started events for every fillable court. lastFinished steers only the first fill. */
function fillEvents(state: SessionState, lastFinished: LastFinished | null = null): CommandEvent[] {
  const out: CommandEvent[] = [];
  let s = state;
  for (let court = 1; court <= s.courtCount; court += 1) {
    if (s.games[court] || s.closedCourts.includes(court)) continue;
    const pairs = nextLineup(s, lastFinished);
    if (!pairs) break;
...
export function finishGame(state: SessionState, court: number, winnerPair?: 0 | 1): CommandEvent[] | null {
  ...
  return [e, ...fillEvents(after, { pairs: active.pairs, winnerPair: e.winnerPair })];
}
