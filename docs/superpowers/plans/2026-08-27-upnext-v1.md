...
export function useSession(): SessionApi {
  ...
  // undo and redo pick their target inside the queue, from the freshest log,
  // so a double tap cannot undo the same event twice
...
interface ButtonProps {
  ...
  block?: boolean;
  onClick?: () => void;
  /** Distinguishes repeated buttons ("Sit out Priya") for assistive tech. */
  ariaLabel?: string;
  children: ReactNode;
  style?: CSSProperties; // must not set background, color, or border: those carry the pressed state
}

export function Button({ variant = 'primary', size = 'md', icon, disabled, block, onClick, ariaLabel, children, style }: ButtonProps) {
...
export function CheckinTile({ name, state = 'out', games, onTap }: {
  ...
  return (
    <button
      type="button" onClick={state === 'playing' ? undefined : onTap} disabled={state === 'playing'}
      onPointerDown={() => setPressed(true)} onPointerUp={() => setPressed(false)} onPointerLeave={() => setPressed(false)} onPointerCancel={() => setPressed(false)}
      style={{
...
export function CourtCard({ court, status, pairs, elapsed, needsWinner, onFinish, onWin, onSwap, onClose, onReopen }: {
  court: number;
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 'var(--space-3)', display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <label style={{ font: '500 14px var(--font-sans)', color: 'var(--blue)', cursor: 'pointer' }}>
              Import session
              <input type="file" accept="application/json" style={{ position: 'absolute', width: '1px', height: '1px', opacity: 0, overflow: 'hidden', clipPath: 'inset(50%)' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) onImport(f); e.target.value = ''; }} />
            </label>
          </div>
          {history.map((h) => (
...
export function SessionBoard({ state, players, undoLabel, canRedo, onUndo, onRedo, onFinish, onWin, onSwap, onCloseCourt, onReopenCourt, onToggleSit, onToggleCheck }: {
  state: SessionState;
  players: Player[];
  undoLabel: string | null;
  canRedo: boolean;
  ...
  return (
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: 'var(--space-4)', padding: 'var(--space-4)', paddingBottom: undoLabel || canRedo ? 'calc(var(--tap-primary) + var(--space-5))' : undefined, alignItems: 'start' }}>
        ...
        {courts.map((n) => {
          ...
          return (
            <CourtCard key={n} court={n} status={status} pairs={pairs} elapsed={fmt(elapsed)} needsWinner={needsWinner}
              onFinish={() => onFinish(n)} onWin={(w) => onWin(n, w)} onSwap={() => onSwap(n)} onClose={() => onCloseCourt(n)} onReopen={() => onReopenCourt(n)} />
          );
        })}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
      ...
      {undoLabel || canRedo ? (
        <div style={{ position: 'fixed', left: 'var(--space-4)', bottom: 'var(--space-4)', zIndex: 5, display: 'flex', gap: 'var(--space-2)', alignItems: 'center' }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setClock(Date.now()); // never show a stale clock on entering the board
...
Note: the plan's Task 10 and 11 snippets were later synced to already carry the swap, redo, and rule-change wiring, so if you implemented them from the current snippets, steps 1 to 3 below are verification rather than new work. The App.tsx additions in steps 2 to 4 remain new.
...
```tsx
<Button variant="ghost" onClick={onSwap} icon="shuffle" ariaLabel={'Swap partners on court ' + court} style={{ alignSelf: 'center' }}>
  Swap partners
</Button>
```
...
- The plan's snippets were synced to the shipped code during execution. On cosmetic placement (attribute order, comment position, prop order) the shipped files are authoritative, not the snippets.
